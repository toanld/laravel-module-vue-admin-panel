...
class PluginServiceProvider extends ServiceProvider
{
    ...
    protected function addMergePluginConfig()
    {
        ...
        if(isset($composer["extra"]["merge-plugin"]["include"])){
            $arrIncludes = $composer["extra"]["merge-plugin"]["include"];
            $arrIncludes = array_flip($arrIncludes);
            $arrIncludes = array_keys($arrIncludes);
            //$composer["extra"]["merge-plugin"]["include"] = $arrIncludes;
            Arr::set($composer, 'extra.merge-plugin.include', $arrIncludes);
        }
